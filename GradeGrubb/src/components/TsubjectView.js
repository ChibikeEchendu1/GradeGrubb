...
import { StyleSheet, SafeAreaView, View, ListView, Text } from "react-native";
		...
		if (this.props.loading) {
			...
			if (this.props.subjects.length == 0) {
				return (
					...
					>
						<LogoHouse />
						<Text>To see subjects you need to first create a subject</Text>
						<Icon key="1" style={{ transform: [{ rotate: "-50deg" }] }} color={"#1995ad"} name="caret-down" size={90} />
					</View>
				);
			} else {
...
const mapStateToProps = state => {
	...
	return { subjects, info: state.pro.info, loading: state.pro.loading1, id: state.pro.id };
};
